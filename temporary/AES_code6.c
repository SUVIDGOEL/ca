...
static inline __attribute__((always_inline)) uint32_t mix_col(uint32_t w) {
    // 1. Rotate the word to align neighboring bytes
    uint32_t r1 = (w >> 8) | (w << 24);
    
    // 2. Compute u = s0^s1^s2^s3 and broadcast it to all 4 bytes
    uint32_t t = w ^ (w >> 8) ^ (w >> 16) ^ (w >> 24);
    t &= 0xFF;
    uint32_t u = t | (t << 8) | (t << 16) | (t << 24);
    
    // 3. Compute v = w ^ r1 (Calculates s0^s1, s1^s2, s2^s3, s3^s0 simultaneously)
    uint32_t v = w ^ r1;
    
    // 4. SWAR XtimeWord logic: Multiply all 4 bytes in 'v' by 2 simultaneously
    uint32_t b = v & 0x80808080u;
    v ^= b;
    b -= b >> 7;
    b &= 0x1B1B1B1Bu;
    b ^= v << 1;
    
    // 5. Final parallel evaluation
    return w ^ u ^ b;
}
...
void AES_encrypt_custom(const unsigned char *plaintext,
    ...
    __m128i key_vl  = _mm_loadu_si128((const __m128i*)&enc_key->rd_key[40]);

    state_v = _mm_xor_si128(state_v, key_vl);
