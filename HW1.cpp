...
unordered_set<ADDRINT> instr_mem;
unordered_set<ADDRINT> data_mem;
UINT32 instr_mem_size = 0;
UINT32 data_mem_size = 0;

vector<UINT32> inst_size(256, 0);
vector<UINT32> inst_operands(256, 0);
vector<UINT32> inst_read_oper(256, 0);
vector<UINT32> inst_write_oper(256, 0);
vector<ADDRINT> inst_memop(256, 0);
vector<ADDRINT> inst_read_cnt(256, 0);
vector<ADDRINT> inst_write_cnt(256, 0);
...
VOID exit_routine(){
    ...
    *outfile << "TOTAL: "               << total << endl;
    ...
    UINT64 total_cycles = ((UINT64)ic._load + ic._store) * 5 +
    ...
    *outfile << "CPI: " << cpi << endl;
    double total_time = cpi * total;
    *outfile << "Total Run Time(in clock cycles): " << total_time << endl;
